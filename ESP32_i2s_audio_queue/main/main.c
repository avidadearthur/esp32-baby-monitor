...
#define SAMPLE_SIZE (BIT_SAMPLE * 128)
 ...
 *        Data from ADC are 12bit width by default and the i2s stream uses 16 bits.
 *        DAC can only output 8 bit data.
 *        Scale each 16bit ADC data to 8bit DAC data.
 *
 * note: Eventhough s_buff is expected to be a 'unint_t *' we pass 'int16_t *' and the code works.
 *
...
/**
 * @brief Record WAV file to SD card.
 */
